...
cat <<EOF | cfssl genkey - | cfssljson -bare server
      {
        "hosts": [
        "kubi-metrics",
        "kubi-metrics.kube-system",
        "kubi-metrics.kube-system.svc",
        "kubi-metrics.kube-system.svc.cluster.local"
         ],
       "CN": "system:node:kubi-metrics.kube-system.svc.cluster.local",
       "key": {
       "algo": "ecdsa",
       "size": 256
         },
...
kubectl certificate approve kubi-metrics.kube-system
kubectl get csr kubi-metrics.kube-system -o jsonpath='{.status.certificate}' | base64 --decode > server.crt
